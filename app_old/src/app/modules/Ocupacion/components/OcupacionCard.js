...
const OcupacionCard = ({ className, title, subtitle }) => {
    ...
    if (selectedKeys !== '') {
      ...
      if (filteredData === '') {
        filteredData = dataSource.filter(
          entry => entry[dataIndex] === selectedKeys
        )
      }
  ...
  const handleButtonClickEdit = row => {
    http.get(`/api/v1/ocupaciones/get/${row.id}`).then(response => {
      setOcupacion([
        {
          id: response.data.ocupacion.id
        },
        {
          name: ['name'],
          value: response.data.ocupacion.name
        }
      ])
      setVisibleEdit(true)
    })
  ...
  const handleButtonClickDelete = id => {
    const props = {
      title: 'Esta seguro de eliminar esta ocupacion?',
  ...
  const onDelete = id => {
    http.delete(`/api/v1/ocupaciones/delete/${id}`).then(response => {
  ...
  const onEdit = values => {
    http
      .put(`/api/v1/ocupaciones/put/${values.id}`, values)
      .then(response => {
        message.success({ content: response.data.message })
        fetch({ pagination })
      })
  ...
  const fetch = (params = {}) => {
    setLoading(true)
    http
      .get('/api/v1/ocupaciones/pagination', { params: getParams(params) })
      .then(response => {
        setPagination({
          ...params.pagination,
          total: response.data.ocupaciones.count
        })
        setData(response.data.ocupaciones.rows)
        setDataSource(response.data.ocupaciones.rows)
        setLoading(false)
      })
  }
  useEffect(() => {
  ...
  return (
    <div>
      <div className={`card card-custom shadow ${className}`}>
              ...
              <Dropdown.Menu className='dropdown-menu dropdown-menu-sm dropdown-menu-right'>
                <ul className='navi navi-hover py-5'>
                  <li className='navi-item'>
                    <ExportBtn
                      nameSheet='Listado de ocupaciones'
                      labels={labels}
                      url={`${process.env.REACT_APP_API_URL}/api/v1/ocupaciones/`}
                    />
                  </li>
                </ul>
              </Dropdown.Menu>
        ...
        />
        <ImportModal
          url={`${process.env.REACT_APP_API_URL}/api/v1/ocupaciones/upload/sheet`}
